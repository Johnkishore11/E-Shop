...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setPrice(parseFloat(prodData.price).toString());
        setStock(parseInt(prodData.stock, 10).toString());
        setCategoryId(prodData.category_id.toString());
        setDescription(prodData.description || "");
        setImageUrl(prodData.image_url || "");
      } catch (err) {
        setError(err.message || "Error fetching editing data");
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [authLoading, token, isAdmin, id, router]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || "Something went wrong saving edits");
      setSubmitting(false);
    }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                placeholder="Item specs details..."
                className="block w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-sm bg-white"
              />
            </div>
          </div>
